...
size_t convert(char buf[], T value){
	...
	*p = '\0';
	std::reverse(buf, p); /*翻转从 buf 开始到 p 结束（不包括 p）的空间中的元素*/
...
{
	double n = static_cast<double>(s);	/*一定要转化为 double 类型*/
	char buf[64];
	if (s < 1000)
...
void LogStream::staticCheck()
{
	...
	/**
	 * std::numeric_limits<double>::digits10 = 15 : 表示使用 double 类型来存储 10 进制数，最多可以存储
	 * 15 位的十进制数而不会发生溢出和精度损失
	 *
	*/
...
void LogStream::formatInteger(T v)
{
	...
	{
		size_t len = convert(buffer_.current(), v);	/*将 v 表示为字符串存到 buffer_ 当中*/
		buffer_.add(len);
	}
}
